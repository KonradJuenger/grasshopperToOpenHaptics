...
namespace ghoh
{
    public class UKF
    {
        ...
        private void init()
        {
            ...
            c = Math.Sqrt(c);
            isInitialized = true;  // Add this line at the end of init()
        }
        ...
        public void SetNoiseParams(double q, double r)
        {
            this.q = q;
            this.r = r;
            Q = Matrix.Build.Diagonal(L, L, q * q);
            R = Matrix.Build.Dense(m, m, r * r);
        }
        ...
        public void Predict()
        {
            ...
            var ut_matrices = UnscentedTransform(X, Wm, Wc, L, Q);
            x = ut_matrices[0];  // Update state
            P = ut_matrices[2];  // Update covariance
        }
        ...
        public void Reset()
        {
            if (isInitialized)
            {
                x = Matrix.Build.Dense(L, 1, 0.0);
                P = Matrix.Build.Diagonal(L, L, 1.0);
            }
        }
        /// <summary>
